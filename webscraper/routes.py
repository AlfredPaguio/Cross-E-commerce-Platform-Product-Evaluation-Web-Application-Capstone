    ...
    if request.method == 'POST':
        ...
        if request.args.get("req") == "search":
            ...
            if what_hostname in supported_sites:
                ...
                if in_dict:
                    flash(f'This product is already on the view, do you mean updating it?', category='info')
        ...
        if request.args.get("req") == "replace_item":
            ...
            if len(session['list_of_products']) > 1:
                ...
                if replace_selected_item:
                    ...
                    flash(f"Unable to swap item: Product view is currently empty", category="danger")
                return redirect(url_for('dashboard_page'))
            else:
...
def forgot_password_page():
    ...
    if forgot_password_form.validate_on_submit():
        ...
        if registered_user.email_address and existing_email:
            ...
            mail.send(msg)
            flash(f'Reset Successful. You may view your email {existing_email.email_address}',
                  category='success')
            return redirect(url_for('login_page'))
        else:
