...
def account_page():
    ...
    return render_template('account.html', change_password_form=change_password_form,
                           maskedEmail=account.mask_email(current_user.email_address))
...
def forgot_password_page():
    ...
    if forgot_password_form.validate_on_submit():
        ...
        if registered_user.email_address and existing_email:
            ...
            mail.send(msg)
            flash(
                f'Reset Successful. You may check your email {account.mask_email(existing_email.email_address)} for '
                f'the new password',
                category='success')
            return redirect(url_for('login_page'))
        else:
